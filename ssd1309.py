...
from framebuf import FrameBuffer, MHMSB, MVLSB#, GS8
...
class Display(object):
    ...
    def __init__(self, spi, cs, dc, rst, width=128, height=64):
        ...
        # Frame Buffer
        self.monoFB = FrameBuffer(self.mono_image, width, height, MVLSB)
        self.clear_buffers()
    ...
    def draw_bitmap(self, path, x, y, w, h, invert=False, rotate=0):
        ...
        with open(path, "rb") as f:
            ...
            if rotate == 0 and invert is True:  # 0 degrees
                fb2 = FrameBuffer(bytearray(array_size), w, h, MHMSB)
                for y1 in range(h):
                    for x1 in range(w):
                ...
                fb2 = FrameBuffer(bytearray(adj_size), h, w, MHMSB)
                for y1 in range(h):
                    for x1 in range(w):
                        if invert is True:
                            fb2.pixel(y1, x1,
